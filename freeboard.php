...
<?php
	...
	if($page != 1) {
		$paging .= '<li class="page page_start"><a href="./freeboard.php?page=1' . $subString . '">처음</a></li>';
	}
	//첫 섹션이 아니라면 이전 버튼을 생성
	if($currentSection != 1) {
		$paging .= '<li class="page page_prev"><a href="./freeboard.php?page=' . $prevPage . $subString . '">이전</a></li>';
	}
	...
	for($i = $firstPage; $i <= $lastPage; $i++) {
		if($i == $page) {
			...
			$paging .= '<li class="page"><a href="./freeboard.php?page=' . $i . $subString . '">' . $i . '</a></li>';
		}
	}

	//마지막 섹션이 아니라면 다음 버튼을 생성
	if($currentSection != $allSection) {
		$paging .= '<li class="page page_next"><a href="./freeboard.php?page=' . $nextPage . $subString . '">다음</a></li>';
	}

	//마지막 페이지가 아니라면 끝 버튼을 생성
	if($page != $allPage) {
		$paging .= '<li class="page page_end"><a href="./freeboard.php?page=' . $allPage . $subString . '">끝</a></li>';
	}
	$paging .= '</ul>';
...
<html>
  ...
  <link rel="stylesheet" type="text/css" href="http://localhost/project/menu.css?ver=1">
    ...
    <article class="boardArticle">
			...
			<table>
				...
				<tbody>
						...
						<tr>
							...
							<td class="title">
								<a href="./freeview.php?bno=<?php echo $row['b_no']?>"><?php echo $row['b_title']?></a>
							</td>
			...
			<div class="btnSet">
				<tr>
					<button type ="submit"><a href="./writefreeboard.php" class="btnWrite btn">글쓰기</a>
				</tr>
			</div>
			<div class="paging">
				<?php echo $paging ?>
