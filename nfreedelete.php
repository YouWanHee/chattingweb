  ...
  <link rel="stylesheet" type="text/css" href="http://localhost/project/menu.css?ver=1">
    ...
    <ul>
      ...
      <article class="boardArticle">
        ...
        <?php
          if(isset($bNo)) {
            $sql = 'select count(b_no) as cnt from noticeboard where b_no = ' . $bNo;
            $result = $db->query($sql);
          }

            $sql = 'select b_title from noticeboard where b_no = ' . $bNo;
            $result = $db->query($sql);
        ?>
        <div id="boardDelete">
          <form action="./nfreedelete_update.php" method="post">
            ...
            <div class="btnSet">
              ...
              <a href="./nfreeboard.php" class="btnList btn">목록</a>
            </div>
          </form>
        </div>
        <script>
